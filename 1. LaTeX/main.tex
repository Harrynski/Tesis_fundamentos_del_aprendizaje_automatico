...
\begin{dfn}
    ...
    de números reales. Llamamos \emph{borelianos} a los elementos de $\mathcal{B}$. 
\end{dfn}
...
\begin{ej}[Esperanza de distribuciones clásicas]
...
\[
V(X) = \operatorname{var}(X) = \sigma^2(X) = \sum_i (x_i - \mathbb{E}X)^2 \, \mathbb{P}(X = x_i).
\]
...
\begin{enumerate}
    ...
    \item \( \operatorname{var}(X) \) minimiza \( \mathbb{E}[(X - a)^2] \).
\end{enumerate}
\end{prop}
...
\[
P(|X| \geq \varepsilon) \leq \frac{1}{\varepsilon^k} E(|X|^k), 
\]
\end{prop}

\begin{prop} \label{prop:chebyshev} (\textbf{Desigualdad de Chebyshev})
\[
P(|X - E(X)| \geq \varepsilon) \leq \frac{1}{\varepsilon^2} \mathrm{Var}(X).
\]
\end{prop}
...
\[
E(g(X)) = \sum_{(x_1,\dots,x_k) \in D} g(x_1,\dots,x_k)f_X(x_1,\dots,x_k),
\]
...
\begin{dfn} \label{dfn:momento_conjunto}  Se define el momento conjunto de orden $(n_1,\dots,n_k)$ como $E(X_1^{n_1} \dots X_k^{n_k})$.
\end{dfn}


\begin{dfn} \label{dfn:momento_conjunto_central}  Se define como $E(X_1 - E(X_1))^{n_1} \dots (X_k - E(X_k))^{n_k}$ para $n_i \geq 0$.
\end{dfn}


\begin{dfn} \label{dfn:covarianza} Supongamos un vector bidimensional $(X_1,X_2)$. Se define la covarianza entre $X_1$ e $X_2$ como
\[
\mathrm{cov}(X_1, X_2) = E(X_1 - EX_1)(X_2 - EX_2).
\]
\end{dfn}

\begin{dfn} \label{dfn:matriz_covarianzas} Dado el vector aleatorio $\mathbf{X} = (X_1,\dots,X_k)$, definimos la matriz de covarianzas como
\[
\Sigma = [\sigma_{ij}]_{i,j=1,\dots,k}
\]
\end{dfn}

\section{Teoremas importantes de la Estadística inferencial}
\subsection{Las Leyes de los Grandes Números}
\begin{thm} \label{thm:ley_debil} (\textbf{Ley débil}) Sea $\{X_k\}$ una sucesión de variables aleatorias independientes 
    tales que $E(X_k^2) < +\infty$, $\forall k$, y $\lim_n \frac{1}{n^2} \sum_{k=1}^n \mathrm{var}(X_k) = 0$, entonces
\[
...
Si $\{X_k\}$ es una sucesión de variables aleatorias i.i.d. con $E(X_1^-) < +\infty$ y $E(X_1^+) = +\infty$, 
...
\[
Y_n = \frac{\bar{X}_n - \mu}{\sigma/\sqrt{n}} = \frac{\bar{X}_n - E(\bar{X}_n)}{\sqrt{\mathrm{var}(\bar{X}_n)}} \xrightarrow{L} N(0,1).
\]
\end{thm}
