...
\begin{titlepage}
    ...
    {\normalsize Trabajo final para optar al título de Licenciado en Matemática \par}
    \vfill
    {\normalsize
...
\vspace*{\fill}
\begin{center}
    \textit{A mis padres, Beatriz y Gustavo.}
\end{center}
\vspace*{\fill}
\newpage
...
\vspace*{\fill}
\begin{flushright}
    \textit{``Examinadlo todo; retened lo bueno.''} \\[0.4em]
    --- 1 Tesalonicenses 5:21
\end{flushright}
\vspace{2cm}
\begin{flushright}
    \parbox{0.75\textwidth}{\raggedleft
        {\itshape ``En aquel Imperio, el Arte de la Cartografía logró tal Perfección que el mapa de una sola Provincia ocupaba toda una Ciudad, y el mapa del Imperio, toda una Provincia. Con el tiempo, estos Mapas Desmesurados no satisficieron y los Colegios de Cartógrafos levantaron un Mapa del Imperio, que tenía el tamaño del Imperio y coincidía puntualmente con él.
        Menos Adictas al Estudio de la Cartografía, las Generaciones Siguientes entendieron que ese dilatado Mapa era Inútil.''}
    } \\[0.8em]
...
Este trabajo explora los fundamentos de la Teoría del Aprendizaje Estadístico. Esta disciplina intenta ser un puente entre la Matemática (especialmente la teoría de la medida
y el análisis funcional) y el Aprendizaje Automático (Machine Learning), creando en un marco riguroso que explica cómo las computadoras pueden extraer reglas generales a partir
de datos empíricos. El campo de la inteligencia artifical, tan en auge en nuestros tiempos, se constituye sobre un andamiaje
de vasta riqueza teórica, pero ha dado sus últimos grandes saltos montado en resultados experimentales y el desarrollo de arquitecturas
...
A pesar de la elegancia de la teoría VC, los últimos hitos de la Inteligencia Artificial presentan un desafío significativo para estos fundamentos clásicos.
...
con capacidad casi infinita no caen simplemente en el sobreajuste (memorización de ejemplos que lleva a la incapacidad de generalizar). Existe una necesidad,
parcialmente mitigada, de adaptar y expandir
...
que pueden ser sorteados por aportes teóricos que habilitan nuevos caminos. Mientras las Scaling Laws (o leyes de escalamiento) se mantengan y los
modelos multimodales que aún llamamos sencillamente ``LLMs'' sigan mejorando según al crecimiento del número de parámetros, el volúmen de datos de entrenamiento
y la capacidad de cómputo, suponemos entendible la prolongación del escaso interés actual en la búsqueda de razonamientos fundacionales para el aprendizaje. Pero
si este progreso como se detiene, o al menos desacelera, la necesidad de una teoría fundamental revisada del aprendizaje puede volverse
...
capaces de mejorarse a si mismas de forma autónoma, quiza la propia Matemática sea la llave para su evolución autoimpuesta.
...
\begin{obs}
    ...
    \begin{equation*}
        F_X(x) = F_X(x; n,p) = P(X\leq x) = \sum_{i=0}^{\lfloor x \rfloor} \binom{n}{i} p^i(1-p)^{1-i}
    \end{equation*}
\end{obs}
...
\[
\lim_{n\rightarrow \infty} P\left(\{\omega : |X_n(\omega) - X(\omega)| > \delta \}\right) = 0.
\]
\end{dfn}
...
\end{dfn}
\begin{obs}
    Notamos \emph{a.s.} a la convergencia casi segura dado su nombre en inglés, \emph{almost surely}.
\end{obs}
...
\begin{obs}
    Notamos \emph{w} a la convergencia débil dado su nombre en inglés, \emph{weakly}.
\end{obs}
...
población con distribución de probabilidad \( P_\theta \) que depende de un parámetro desconocido \( \theta \in \Theta \), con \(\Theta \) un espacio de parámetros usualmente escalar. El objetivo es obtener información
...
\chapter[Introducción a la Teoría del Aprendizaje Estadístico]{Un acercamiento amistoso a la Teoría del Aprendizaje Estadístico}
...
\(f: \mathcal{X}\rightarrow \mathcal{Y}\), que comete la menor cantidad de errores posibles. A esta función \(f\) la llamamos \textit{clasificador}.

\section{La historia del aprendizaje automático}

\subsection{Los orígenes de la Teoría del Aprendizaje}

El primer modelo de aprendizaje automático, según Vapnik en \cite{vapnik1995}, fue sugerido por F. Rosenblatt, un psicólogo estadounidense, al que llamó \textit{perceptrón}, y su introducción constituye el comienzo del análisis matemático del aprendizaje. Conceptualmente, la idea del perceptrón no era nueva, estando presente en la literatura de Neurofisiología durante varios años. Rosenblatt, sin embargo, se aventuró en describir el modelo como un programa para computadoras y demostró con simples experimentos que dicho modelo era generalizable. El perceptrón fue construido como una solución a un problema particular dentro del aprendizaje automático, el reconocimiento de patrones. En el caso más sencillo, este problema consiste en hallar una regla para separar datos en dos categorías distintas a partir de ejemplos.

Para construir la regla de separación, el perceptrón sigue el modelo más sencillo de neurona, propuesto previamente por McCulloch y Pitts, de acuerdo al cual una neurona recibe \(n\) valores (o \textit{inputs}) en la forma de un vector \(x = (x^1,\dots, x^n) \in \mathcal{X} \subset \mathbb{R}^n \) y genera una etiqueta (\textit{output}) \(y\in\{-1,+1\}\) a través de una dependencia funcional dada por:

\begin{equation}
    y = \text{sgn}\{(w \cdot x) - b\}
\end{equation}

con \(\cdot\) el producto interno de vectores en \(\mathbb{R}^n\), \(b\) un valor de límite y un vector \(w\) que se genera en el proceso de aprendizaje. Geométricamente, una neurona divide el espacio \(\mathcal{X}\) en dos regiones: en una la etiqueta \(y\) vale \(+1\) y en la otra \(-1\). Las dos regiones son separadas por el hiperplano:

\begin{equation}
    (w \cdot x) - b = 0
\end{equation}

El vector \(w\) y el escalar \(b\) determinan la posición del hiperplano y sus valores son aprendidos por el perceptrón. Para hallar estos coeficientes de manera iterativa, Rosenblatt propuso la siguiente regla de actualización para cada muestra mal clasificada $(x_i, y_i)$:

\begin{align}
    w^{(t+1)} &\gets w^{(t)} + \eta (y_i - \hat{y}_i) x_i \\
    b^{(t+1)} &\gets b^{(t)} - \eta (y_i - \hat{y}_i)
\end{align}

donde $\hat{y}_i$ es la predicción actual y $\eta \in (0, 1]$ es un valor ajustable (lo que llamámos hiperparámetro), la tasa de aprendizaje (o \textit{learning rate}).
Cuando combinamos varias neuronas, el perceptrón separa el espacio de entrada en dos regiones lineales a trozos y no necesariamente conexas.
En 1960 no era claro cómo elegir todos los parámetros \((w_1,\dots,w_k)\) y \((b_1,\dots, b_k)\) de todas las neuronas, por lo que se fijaban los valores de las
primeras \(k-1\) y se intentaba encontrar los valores deseables para la última de ellas. Geométricamente, se transformaba el espacio de entrada $X$ en un nuevo espacio $Z$
(eligiendo coeficientes apropiados para las primeras $k-1$ neuronas) y luego se utilizaban los datos de entrenamiento para construir un hiperplano que separe el plano $Z$.
...
\begin{enumerate}
    \item La norma de los vectores de entrenamiento $z_i$ está acotada por una constante $R$:
    \[ |z_i| \leq R, \qquad \forall i=1,2,\dots,k \]
    \item Los datos de entrenamiento pueden ser separados con un margen $\rho$:
    \[ \sup_{w} \min_{i} y_i(z_i \cdot w) > \rho \]
    \item Los datos son alimentados al perceptrón una cantidad \textit{suficiente} de veces.
\end{enumerate}
...
\subsection[Arquitecturas modernas en NLP]{Arquitecturas modernas en NLP:\\Transformers, BERT y GPT}
...
en un espacio de entrada $\mathcal{X}$ con alguna de las dos posibles etiquetas del espacio $\mathcal{Y}=\{-1,+1\}$. Consideremos,
de ahora en más, a las variables aleatorias $X$ e $Y$ que representan el tomado de muestras de estos espacios, es decir,
las funciones medibles que describen la probabilidad de obtener determinados valores en cada uno de los conjuntos. Aquí, \textit{aprender}
...
El ejemplo que hemos dado del perceptrón de Rosenblatt previamente es un caso de un algoritmo de aprendizaje.
...
\section{El clasificador de los \texorpdfstring{$k$}{k} vecinos más cercanos}
...
que los clasificadores lineales de este tipo tienen un coeficiente de fragmentación de $2^3=8$ (máximo posible) para muestras de tres puntos en $\mathbb{R}^2$.
...
se hace presente en las muestras que tomamos. En lo siguiente, representará al riesgo
empírico.
...
\[
\begin{aligned}
\quad C &= \left\{ \rho^{(l)} \geq \frac{1}{2}\varepsilon \right\}
...
\begin{equation*}
\begin{aligned}
P^{(2l)}(C) &= P^{(2l)}\left(\rho^{(l)}-\frac{\varepsilon}{2}\geq 0\right) \\
&= \int_{\Omega^{(2l)}} \left( 1 \cdot \chi_{\{\rho^{(l)}-\frac{\varepsilon}{2}\geq 0\}}
...
\begin{obs}\label{obs:desigualdad triangular indice de un sistema}
    ...
    \begin{equation*}
    \begin{aligned}
    \log_2 \Delta^{\mathcal{S}}(\omega_1, \dots, \omega_{k}, \omega_{k+1}, \dots,\omega_l)
    &\leq \log_2 \Delta^{\mathcal{S}}(\omega_1, \dots, \omega_k) \\
    &\quad + \log_2 \Delta^{\mathcal{S}}(\omega_{k+1}, \dots, \omega_l).
    \end{aligned}
    \end{equation*}
\end{obs}
...
\begin{proof}
    ...
    &= P^{(l)}\left(\left\{\{\omega_1,\ldots,\omega_l\}\in\Omega^{(l)}:\right.\right. \\
    &\qquad \left.\left. y(\omega_1,\ldots,\omega_l) - c > \frac{\varepsilon}{2} \right\}\right).
    \end{aligned}
    \]
    ...
    \[
    \begin{aligned}
    \left\{\{\omega_1,\ldots,\omega_l\}: y(\omega_1,\ldots,\omega_l) - c > \frac{\varepsilon}{2} \right\}
    &\subseteq \left\{\{\omega_1,\ldots,\omega_l\}:\right.\\
    &\qquad \left.y(\omega_1,\ldots,\omega_l) - \frac{H^{\mathcal{S}}(l_0)}{l_0} > \frac{\varepsilon}{4} \right\}.
    \end{aligned}
    \]
