...
no podemos calcular el riesgo \(R(f)\). Sin embargo, lo que sí conocemos son los datos de entrenamiento, 
...
\(R(f)\), intentamos encontrar aquella que minimice el riesgo empírico:
...
\[
f_n := \arg\min_{f \in F} R_{\text{emp}}(f).
\]
...
\[
R_{\text{emp}}(f) = \frac{1}{n} \sum_{i=1}^n \ell(X_i, Y_i, f(X_i)) \to \mathbb{E}(\ell(X, Y, f(X))) \quad 
...
\[
P(|R_{\text{emp}}(f) - R(f)| \geq \epsilon) \leq 2 \exp(-2n\epsilon^2). \tag{II}
\]
...
riesgo empírico \(R_{\text{emp}}(f_n) = 0\). En consecuencia, dado que el riesgo empírico no puede ser 
...
\begin{figure}[h]
    ...
    infinito, el riesgo empírico \(R_{\text{emp}}(f)\) converge al riesgo verdadero \(R(f)\) (indicado por la flecha). 
    ...
    Para que esto sea cierto, requerimos la convergencia uniforme de \(R_{\text{emp}}(f)\) a \(R(f)\) sobre todas 
...
del riesgo empírico consiste en elegir la función \(f\) que minimiza \(R_{\text{emp}}\). Es consistente si el 
mínimo de \(R_{\text{emp}}\) converge al de \(R\) a medida que el tamaño de la muestra aumenta. Una forma de 
garantizar la convergencia del mínimo para todas las funciones en \(\mathcal{F}\) es la \textbf{convergencia uniforme} sobre 
\(\mathcal{F}\): requerimos que para todas las funciones \(f \in \mathcal{F}\), la diferencia entre \(R(f)\) y \(R_{\text{emp}}(f)\) 
...
\[
\sup_{f \in \mathcal{F}} |R(f) - R_{\text{emp}}(f)| \leq \epsilon.
\]
...
\[
P(|R(f_n) - R_{\text{emp}}(f_n)| \geq \epsilon) \leq P\left(\sup_{f \in \mathcal{F}} |R(f) - R_{\text{emp}}(f)| \geq \epsilon\right). \tag{III}
\]
...
\[
P\left(\sup_{f \in \mathcal{F}} |R(f) - R_{\text{emp}}(f)| \geq \epsilon \right) \to 0 \quad \text{cuando } n \to \infty.
\]
...
Para verlo, consideremos el siguiente desarrollo:
...
\[
R(f_n) - R_{\text{emp}}(f_n) + R_{\text{emp}}(f_n) - R_{\text{emp}}(f_\mathcal{F}) + R_{\text{emp}}(f_F) - R(f_\mathcal{F}).
\]
\[
    \leq R(f_n) - R_{\text{emp}}(f_n) + R_{\text{emp}}(f_\mathcal{F}) - R(f_\mathcal{F}).
\] 
\[
    \leq 2 \sup_{f \in F} |R(f) - R_{\text{emp}}(f)|.
\]
Dado que \(R_{\text{emp}}(f_n) - R_{\text{emp}}(f_\mathcal{F}) \leq 0\) por definición de \(f_n\), y luego acotando
al tomar el supremo de todas las funciones en el espacio $\mathcal{F}$.\\

De esto podemos concluir:

\[
P\left(|R(f_n) - R(f_\mathcal{F})| \geq \epsilon \right) \leq P\left(\sup_{f \in \mathcal{F}} |R(f) - R_{\text{emp}}(f)| \geq \epsilon/2\right).
\]
...
La convergencia uniforme
    ...
    \[
        P\left(\sup_{f \in F} |R(f) - R_{\text{emp}}(f)| > \epsilon \right) \to 0 \quad \text{cuando } n \to \infty, 
        \]
...
Por lo tanto, cuanto mayor sea \(\mathcal{F}\), mayor será 

\[
P\left(\sup_{f \in F} |R(f) - R_{\text{emp}}(f)| > \epsilon \right).
\]
