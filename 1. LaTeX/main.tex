...
Donde integramos sobre la muestra de tamaño $2l$ de tal manera que
\begin{equation}
...
Retomando la desigualdad (\ref{desigualdad P(C)}), reparemos un momento en la integral del interior. Dada la monotonía de $\theta$ y quedándonos
con tan solo un evento $A_0\in\mathcal{S}$, podemos escribir
...
En donde en la última desigualdad aplicamos lo recién explicado. Ahora notemos que 
\[
...
Ahora, utilizaremos la desigualdad de Chebyshev 
\[
P\left\{|X- \mathbb{E}\left(X \right)| \geq k \cdot \sqrt{\text{Var}(X)} \right\} \leq \frac{1}{k^2}
\]

Tomando la variable aleatoria $X=\nu_{A_0}^{(2)}$ y 
\[
...
Podemos escribir la frencuencia relativa del evento $A_0$ sobre la muestra $Z^{(l)}_2=\{X_1,\dots,X_l\}$ como la suma de variables aleatorias
binomiales $\tilde{X}_i$ con $i=1,2,\dots,l$ tales que
...
Es decir, la frecuencia relativa es una combinación lineal de variables binomiales, cada una con probabilidad $P_{A_0}$ (que no es ni mas ni
...
\[
\begin{aligned}
    \mathbb{E}\left(\nu_{A_0}^{(2)} \right) &= \mathbb{E}\left(\frac{n_{A_0}^{(2)}}{l} \right) =  \frac{1}{l} \cdot \mathbb{E}\left( \#\left\{X_i\in Z_2^{(l)}\mid X_i\in A_0\right\}\right)\\
...
recordemos que la varianza de toda variable aleatoria binomial tiene forma
...
\leq \frac{4(1 - P_{A_0}) P_{A_0}}{\varepsilon^2 l} < \frac{1}{\varepsilon^2 l}.
\]\newline

Por lo tanto, para \( l \geq 2 / \varepsilon^2 \),

\[
\int_{Z_2^{(l)}} \theta \left( \frac{\varepsilon}{2} - \left| \nu^{(2)}_{A_0} - P_{A_0} \right| \right) dP_2 > \frac{1}{2}.
\]
